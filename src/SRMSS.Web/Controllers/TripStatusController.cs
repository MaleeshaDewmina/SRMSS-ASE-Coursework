...
namespace SRMSS.Web.Controllers
{
    ...
    public class TripStatusController : Controller
    {
            ...
            string? search)
        {
                ...
                schedulesQuery =
                    schedulesQuery.Where(
                        s =>
                            ...
                            (
                                s.Driver != null &&
                                s.Driver.FullName.Contains(cleanSearch)
                            )
                            ||
                            (
                                s.Vehicle != null &&
                                s.Vehicle.RegistrationNumber.Contains(cleanSearch)
                            )
                    );
            }
            ...
            ViewBag.DepartedTrips =
                await _context.Schedules.CountAsync(
                    s => s.Status == "Departed"
                );

            ViewBag.DelayedTrips =
                await _context.Schedules.CountAsync(
                    s => s.Status == "Delayed"
                );

            ViewBag.CompletedTrips =
                await _context.Schedules.CountAsync(
                    s => s.Status == "Completed"
                );

            ViewBag.CancelledTrips =
                await _context.Schedules.CountAsync(
